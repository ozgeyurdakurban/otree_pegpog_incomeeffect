...
class Constants(BaseConstants):
    name_in_url = 'TPOGcont'
    players_per_group = 2
    num_rounds = 10
    instructions_template = 'TPOGcont/instructions.html'
    table_template = 'TPOGcont/table.html'
...
class Subsession(BaseSubsession):
    endowment_Decider = models.IntegerField(initial=0)
    maxtosend = models.IntegerField(initial=0)
...
class Group(BaseGroup):
    ...
    expect_decider = models.CurrencyField(
        #    min=0, max=Subsession.endowment_Decider-2,
        doc="""Amount expected by P1""",
    )
    sent_back_amount = models.CurrencyField(
        doc="""Amount sent back by P2""",
        #    min=c(0), max=Subsession.endowment_Decider-2,
    )
...
def creating_session(subsession: Subsession):
    subsession.endowment_Decider = random.choice([6, 9])
    subsession.maxtosend = subsession.endowment_Decider - 2
    #        for group in self.get_groups():
    #           print(group)
    #           print(group.
    #            print(group.sent_amount)
    # group.sent_amount(max)=self.maxtosend
...
def set_payoffs(group: Group):
    ...
    p1.payoff = (group.subsession.endowment_Decider - group.sent_amount) * (
        Constants.endowment_Receiver + group.sent_back_amount
    )
    p2.payoff = (group.subsession.endowment_Decider - group.sent_back_amount) * (
...
class Send(Page):
    ...
    def vars_for_template(player: Player):
        return {
            ...
            'prompt': '0 ile {} arasında bir sayı giriniz.'.format(
                player.subsession.endowment_Decider - 2
            ),
        }
...
class Results(Page):
    ...
    def vars_for_template(player: Player):
        return {
            'round_number': '{}'.format(player.round_number),
            'player1_period1_amount': player.subsession.endowment_Decider
            ...
            'player2_period2_amount': player.subsession.endowment_Decider
            - player.group.sent_back_amount,
            'player1_payoff': (player.subsession.endowment_Decider - player.group.sent_amount)
            ...
            * (player.subsession.endowment_Decider - player.group.sent_back_amount),
        }
