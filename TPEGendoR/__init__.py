...
class Constants(BaseConstants):
    name_in_url = 'TPEGendoR'
    players_per_group = None
    num_rounds = 10
    instructions_template = 'TPEGendoR/instructions.html'
    table_template = 'TPEGendoR/table.html'
...
def assign_sequence(subsession: Subsession):
    ...
    for index in range(2, 8):  # self.session.config['num_demo_participants']
        pl.session_random_number = rand_num
    ...
    for pl in blue_players + red_players:
        pl.sequence = subsession.session.config['num_demo_participants']
        pl.session_random_number = rand_num
